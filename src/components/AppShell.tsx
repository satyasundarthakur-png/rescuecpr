...
function PaletteTrigger({ collapsed, onClick }: { collapsed: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-3 rounded-lg text-sm font-medium text-white/55 hover:bg-white/10 hover:text-white transition-all mb-2 ${collapsed ? 'justify-center px-2 py-2.5' : 'px-3 py-2'}`}
      title="Search (⌘K)"
    >
      <span className="flex items-center justify-center h-7 w-7 rounded-md text-white/50">
        <Search size={15} />
      </span>
      {!collapsed && (
        <span className="flex-1 flex items-center justify-between">
          Search
          <kbd className="text-[10px] font-mono text-white/40 border border-white/20 rounded px-1.5 py-0.5">⌘K</kbd>
        </span>
      )}
    </button>
  )
    ...
    <div className="min-h-screen flex">
      ...
      <aside className={`shrink-0 bg-gradient-to-b from-clinical-700 to-clinical-800 flex flex-col transition-all duration-200 ${collapsed ? 'w-[76px]' : 'w-60'}`}>
        ...
        <nav className="flex-1 px-2 py-4 space-y-1">
          <PaletteTrigger collapsed={collapsed} onClick={() => setPaletteOpen(true)} />
